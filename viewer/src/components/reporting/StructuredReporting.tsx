...
const StructuredReporting: React.FC<StructuredReportingProps> = ({
  ...
  // SIMPLIFIED Workflow state - Single source of truth
  ...
  const [creationMode, setCreationMode] = useState<'normal' | 'ai' | 'template' | null>(null)
  ...
  // CLEAN: Handle creation mode selection
  const handleModeSelection = useCallback((mode: 'normal' | 'ai' | 'template') => {
    setCreationMode(mode)
    addAuditEntry(`Report creation mode: ${mode}`)
    ...
    switch (mode) {
      ...
      case 'ai':
        // Auto-select template and go to editor
        const modality = studyData.modality || 'XA'
        const template = availableTemplates.find(t => t.modality === modality) || availableTemplates[0]
        if (template) {
          setSelectedTemplate(template)
      ...
      case 'template':
        setWorkflowStep('template-browser')
        break
    }
  }, [studyData, availableTemplates, addAuditEntry])

  // Trigger AI generation when in editor with AI mode
  useEffect(() => {
    if (isGeneratingReport && selectedTemplate && creationMode === 'ai' && workflowStep === 'editor') {
      generateAIReport()
    }
  }, [isGeneratingReport, selectedTemplate, creationMode, workflowStep])
  ...
  if (workflowStep === 'template-browser') {
    return (
      <Box sx={{ 
        width: '100%', 
        height: '100%', 
          ...
          <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Box>
              ...
              <Typography variant="body1" sx={{ color: '#666' }}>
                Choose a template that matches your study type
              </Typography>
            </Box>
            <Button 
              variant="outlined" 
              ...
              <Grid item xs={12} sm={6} md={4} key={template.id}>
                <Card 
                  ...
                  onClick={() => {
                    ...
                    setReportSections(initialSections)
                    setWorkflowStep('editor')
                    addAuditEntry(`Template selected: ${template.name}`)
                  }}
                >
                  <CardContent>
                    <Box sx={{ 
                      ...
                      <TemplateIcon sx={{ fontSize: 32, color: '#2196f3' }} />
                    </Box>
                    <Typography variant="h6" sx={{ color: '#1976d2', fontWeight: 700, mb: 1, textAlign: 'center' }}>
                      {template.name}
                    </Typography>
                    <Chip 
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                      {template.sections.slice(0, 3).map(section => (
                        <Typography key={section.id} variant="caption" sx={{ color: '#999' }}>
                          • {section.title}
                        </Typography>
                      ))}
  ...
  return (
    <Box sx={{ width: '100%', height: '100%', bgcolor: '#1a1a1a', color: '#fff', position: 'relative' }}>
      ...
      {/* STEP 3: Quick Reports Dialog */}
      <Dialog
        open={workflowStep === 'quick-reports'}
        onClose={() => setWorkflowStep('selection')}
        maxWidth="md"
        fullWidth
      >
