...
class ReportExportService {
  ...
  public async exportToPDF(reportData: ReportData, options: ExportOptions = { 
    ...
    try {
      const doc = new jsPDF()
      let yPosition = 30 // Start lower to make room for header
      const pageWidth = doc.internal.pageSize.getWidth()
      const pageHeight = doc.internal.pageSize.getHeight()
      const margin = 20
      let currentPage = 1

      // Helper function to add hospital header
      const addHeader = () => {
        const headerY = 10;
        
        // Hospital name (use provided or default)
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(0, 51, 102);
        doc.text(options.headerInfo?.institutionName || 'Medical Center', margin, headerY);
        
        // Hospital details
        doc.setFontSize(8);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(100);
        const hospitalInfo = options.headerInfo?.departmentName || 'Radiology Department';
        doc.text(hospitalInfo, margin, headerY + 5);
      ...
      const addFooter = () => {
        ...
        doc.setTextColor(100);
        doc.text('Generated by computer - No signature required', pageWidth / 2, footerY, { align: 'center' });
      ...
      const addHeaderFooter = () => {
        ...
        doc.setTextColor(100);
        const hospitalInfo = [
          options.headerInfo?.departmentName || 'Radiology Department',
          options.headerInfo?.physicianName ? `Radiologist: ${options.headerInfo.physicianName}` : ''
        ].filter(Boolean).join(' | ');
        doc.text(hospitalInfo, margin, headerY + 5);
      ...
      Object.entries(reportData.sections).forEach(([sectionId, content]) => {
        if (content && content.trim()) {
          ...
          const lines = doc.splitTextToSize(decodedContent, pageWidth - 2 * margin)
